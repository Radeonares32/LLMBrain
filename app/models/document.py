...
class Document(BaseModel):
    ...
    file_type: str = Field(..., description="Extension or special filename, e.g. '.py', 'Dockerfile'.")
    language: str = Field(default="unknown", description="Detected programming language.")
    line_count: int = Field(default=0, ge=0)
    size_bytes: int = Field(default=0, ge=0)
    content: Optional[str] = Field(default=None, description="Raw content (may be omitted for large files).")
